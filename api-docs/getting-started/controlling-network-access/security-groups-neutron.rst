...
and rules by using the neutron client.
...
#. Issue the following neutron client command.
   ...
   **Positional arguments:**
   ...
   -  ``description`` The description of the security group. In this example,
      the description is ``"security group for webservers"``.

   -  The name of the security group. In this example, the name is
      ``1-new-tcp``.
...
#. Issue the following neutron client command, substituting your own
   ...
   **Positional arguments:**
   ...
   -  ``direction`` The direction in which the security group rule is applied.
      In this example, we used direction ``ingress``.
   -  ``ethertype`` Must be IPv4 or IPv6. In this example, we used the
      ethertype ``IPv4``.
   -  ``port-range-min`` The minimum port number in the range that is matched
      by the security group rule. In this example, we used the minimum ``22``.
   -  ``port-range-max`` The maximum port number in the range that is matched
      by the security group rule. In this example, we used the maximum ``22``.
   ...
   -  ``protocol`` The protocol that is matched by the security group rule.
      Valid values are ``null``, ``tcp``, ``udp``, and ``icmp``. In this
      example, we used the protocol ``tcp``.
...
#. To list server details, issue the following nova client command,
   substituting your server ID for the ones shown.
   ...
   **Positional argument:**
   ...
   -  ``id``. The ID of the server to which you want to add the security group.
      In this example, the server ID is
...
#. Because you will apply the security group to the PublicNet port (security
   groups can only be applied to server ports on PublicNet or ServiceNet), note
   the public IP address (``addr``) value from the output. In this example, the
   IP address is ``10.22.253.123``, but use the IP address from your response.
...
#. Note the ``id`` of the port that contains the target port IP address that
   you identified. In this example, the port ID is
...
#. To apply the group (in this example
   ...
   neutron client command, substituting your port ID and security group for the
   ones shown.
   ...
   **Positional arguments:**
   ...
   -  ``security-group``. The security group. In this example, the security
      group ID is ``fd269d7a-0909-488f-b5fc-5606c8aa489f``.
...
#. To add the rule, issue the following neutron client command, substituting
   ...
   **Positional arguments:**
   ...
   -  ``ethertype`` Must be IPv4 or IPv6. In this example, we used the
      ethertype ``IPv4``.
   -  ``port-range-min`` The minimum port number in the range that is matched
      by the security group rule. In this example, we used the minimum
      ``null``.
   -  ``port-range-max`` The maximum port number in the range that is matched
      by the security group rule. In this example, we used the maximum
      ``null``.
   -  The name or ID of the network. In this example, we use the network ID,
      which is ``fd269d7a-0909-488f-b5fc-5606c8aa489f``.
   -  ``protocol` The protocol that is matched by the security group rule.
      Valid values are ``null``, ``tcp``, ``udp``, and ``icmp``. In this
      example, we used the protocol ``icmp``.
...
#. Note the security group rule ID value from the output. You will need this
   value for verification. In this example, the ID is
...
#. Notice that your group, ``fd269d7a-0909-488f-b5fc-5606c8aa489f``, contains
   both your rules, ``483b107a-dbf8-41a9-8494-f47558b58524`` and
...
#. To apply the group, in this example
   ``fd269d7a-0909-488f-b5fc-5606c8aa489f``, to the port, issue the following
   neutron client command, substituting your port ID for the one shown.
